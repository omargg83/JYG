<?php 
	require_once("personal_db.php");
	$personal = new Personal();
	
	echo "<div class='container' style='background-color:".$_SESSION['cfondo']."; '>";
		if ($personal->nivel_personal==0 or $personal->nivel_personal==11){
			$sql="select * from area where tipo<=3 order by area.orden";
		}
		if ($personal->nivel_personal==1){
			$sql="select * from area where tipo<=3 order by area.orden";
		}
		if ($personal->nivel_personal==2){
			$sql="select * from area where idarea='".$_SESSION['idarea']."' or sub='".$_SESSION['idarea']."'";
			$areas=$personal->general($sql);
			$listaar="";
